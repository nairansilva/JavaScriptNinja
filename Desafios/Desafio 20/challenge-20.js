...
$button.addEventListener('click', (event) => {
    ...
    if ($inputEmail.value === '')
        ...
        message = isValidEmail($inputEmail.value);
    ...
    if (message !== '' && message !== undefined) {
        ...
        var confirmaFormulario = confirm("Tem certeza que deseja enviar o formulário?")

        if (confirmaFormulario) 
...
//((([a-zA-Z\d_\.\+^@]))(@[\w_]+)(.\w{2,}))\.?(\w{2})?
var regex = /^[\w+.]+@\w+\.\w{2,}(\.\w{2})?$/
    if (!regex.test(email))
        return "E-mail fora do padrão."
}
})(document);